...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${course} ${year} Notes | pharmA2G`,
...
export default async function NotesSubjectPage({ params }: Props) {
    ...
    const subjects = [...new Set(notes.filter(n => n.course === decodedCourse && n.year === decodedYear).map(n => n.subject).filter(Boolean))];

    return (
        <div className="container mx-auto px-4 py-12">
            <h1 className="text-3xl font-bold">Course: {decodedCourse} - {decodedYear}</h1>
            <p className="mt-2 text-muted-foreground">
                Select a subject to find notes.
            </p>
            <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
                {subjects.map(subject => (
                    <Link key={subject} href={`/notes/${params.course}/${params.year}/${encodeURIComponent(subject)}`} className="p-6 border rounded-lg hover:bg-secondary">
