...
function NoteCard({ note }: { note: Note }) {
  return (
    <div className="group bg-white rounded-[1.5rem] border border-slate-200 p-5 shadow-sm hover:shadow-xl hover:border-indigo-400 hover:-translate-y-1 transition-all duration-300 flex flex-col h-full relative overflow-hidden cursor-pointer active:scale-[0.97]">
      {/* Decorative Corner */}
      <div className={`absolute top-0 right-0 w-12 h-12 ${note.color || 'bg-slate-500'} opacity-5 rounded-bl-[3rem] transition-all group-hover:scale-150`} />
      
      <div className="flex justify-between items-start mb-4">
        <div className={`w-12 h-12 rounded-2xl ${note.color || 'bg-slate-500'} flex items-center justify-center text-white shadow-lg shadow-current/20`}>
          <BookOpen size={24} />
        </div>
        {note.isPremium && (
      ...
      <div className="pt-4 border-t border-slate-100 flex items-center justify-between mt-auto">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-black text-slate-400 uppercase">Archive No. {note.id.substring(0, 4)}</span>
        </div>
...
export default function NotesPageClient({ initialNotes }: { initialNotes: Note[] }) {
  ...
  const [openCourse, setOpenCourse] = useState('B.Pharm'); // Default open
  const [openYear, setOpenYear] = useState(null);

  const yearOrder: { [key: string]: number } = { "1st Year": 1, "1st year": 1, "2nd Year": 2, "3rd Year": 3, "4th Year": 4, "2024 Edition": 5 };
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5] text-[#1e293b] pb-24 font-sans antialiased">
      ...
      <main className="max-w-4xl mx-auto px-4 mt-6 space-y-6">
        ...
        {Object.entries(organizedData).sort().map(([courseName, years]: [string, any]) => (
              ...
              {openCourse === courseName && (
                <div className="p-4 space-y-4 bg-[#F8FAFC]">
                  {Object.entries(years)
                    ...
                    .map(([yearName, subjects]: [string, any]) => (
                      <div key={yearName} className="rounded-3xl border border-slate-200 bg-white overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                        <button 
                             ...
                             {Object.entries(subjects).map(([subjectName, notes]: [string, any]) => (
                               <div key={subjectName} className="mb-8 last:mb-0">
                                  <div className="flex items-center gap-3 mb-4 px-2">
                                    ...
                                    <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] whitespace-nowrap">{subjectName}</h3>
                                    <div className="h-[2px] flex-1 bg-slate-200" />
                                  </div>
