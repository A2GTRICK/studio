...
export default function NotesPageClient({ initialNotes }: { initialNotes: Note[] }) {
  ...
  return (
    <div className="min-h-screen bg-[#F0F2F5] text-[#1e293b] pb-24 font-sans selection:bg-indigo-100">
      {/* Header Section */}
        ...
        <div className="max-w-4xl mx-auto px-4 py-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-100 border border-transparent rounded-2xl py-3 pl-12 pr-4 focus:bg-white focus:ring-4 focus:ring-indigo-500/10 outline-none font-semibold text-sm transition-all"
              value={searchQuery}
      ...
      <main className="max-w-4xl mx-auto px-4 mt-6 space-y-6">
        ...
        {Object.entries(organizedData)
          .sort()
          .map(([courseName, years]) => (
            <div key={courseName} className="relative group">
              <div
                className={`rounded-[2rem] border-2 bg-white shadow-xl shadow-slate-200/50 overflow-hidden transition-all duration-300 ${
                  openCourse === courseName ? 'border-transparent' : 'border-white'
                }`}
              >
                <button
                  onClick={() => toggleCourse(courseName)}
                  className={`w-full flex items-center justify-between p-6 text-left transition-all border-l-[6px] ${
                  ...
                  <div className="flex items-center gap-4">
                    <div
                      className={`p-3 rounded-2xl shadow-inner ${
                        openCourse === courseName ? 'bg-white text-indigo-600' : 'bg-white/50 text-slate-500'
                      }`}
                    >
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <ChevronDown
                      ...
                      className={`transition-transform duration-500 ${
                        openCourse === courseName ? 'rotate-180 text-indigo-600' : 'text-slate-300'
                      }`}
                    />
                  </div>
                </button>
                ...
                {openCourse === courseName && (
                  <div className="p-4 space-y-4 bg-[#F8FAFC]">
                    {Object.entries(years as { [key: string]: any })
                        ...
                        >
                          ...
                          >
                            <div className="flex items-center gap-4">
                              <div
                                className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                                  openYear === yearName ? 'bg-indigo-600' : 'bg-slate-100 text-slate-400'
                                }`}
                              >
                                <Calendar size={18} />
                              </div>
                              <div>
                                ...
                                <div
                                  className={`text-[10px] font-black flex items-center gap-2 tracking-tight ${
                                    openYear === yearName ? 'text-indigo-200' : 'text-slate-400'
                                  }`}
                                >
                            ...
                            <ChevronDown
                              ...
                              className={`transition-transform duration-300 ${
                                openYear === yearName ? 'rotate-180 text-white' : 'text-slate-300'
                              }`}
                            />
                          </button>
                          ...
                          {openYear === yearName && (
                            <div className="p-4 bg-[#F1F5F9]">
                              {Object.entries(subjects as { [key: string]: any }).map(([subjectName, notes]) => (
...
function NoteCard({ note }: { note: Note }) {
  return (
    <div className="group bg-white rounded-[1.5rem] border border-slate-200 p-5 shadow-sm hover:shadow-xl hover:border-indigo-400 hover:-translate-y-1 transition-all duration-300 flex flex-col h-full relative overflow-hidden cursor-pointer active:scale-[0.97]">
      <div
        className={`absolute top-0 right-0 w-12 h-12 ${note.color || 'bg-slate-500'} opacity-5 rounded-bl-[3rem] transition-all group-hover:scale-150`}
      />

      <div className="flex justify-between items-start mb-4">
        <div
          className={`w-12 h-12 rounded-2xl ${note.color || 'bg-slate-500'} flex items-center justify-center text-white shadow-lg shadow-current/20`}
        >
          <BookOpen size={24} />
        </div>
        {note.isPremium && (
